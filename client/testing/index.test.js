...
let index = require('../static/index');
...
describe('helpers', () => {
    ...
    describe('mode helpers', () => {
        ...
        describe('Page loads', () => {
            test('Page has given class', async () => {
                await index.fetchData("https://api.giphy.com/v1/gifs/search?api_key=y64uvNhCUvczuf2GxjoZnEJW5C3IxlV1&limit=1&q=hello")
                body = document.querySelector('body')
                gifLoaded = document.getElementById("gifFetched")
                expect(body.className).toBe("randomTest")
            ...
            test('Page has given class', async () => {
                await index.init("hello")
                body = document.querySelector('body')
                gifLoaded = document.getElementById("gifFetched")
                expect(body.className).toBe("randomTest")
                expect(gifLoaded.id).toBe("gifFetched")
            })
            test('Page has given class', () => {
                let content = {
                    "data": [
                        {
                            "type": "gif",
                ...
                expect(index.createImage(content)).toBe("done")
            })
        })
